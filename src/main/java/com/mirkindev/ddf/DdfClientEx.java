...
/**
 * Created by dimam on 06/11/2016.
 */
public class DdfClientEx implements DdfClient {

    private static final Logger log = LoggerFactory.getLogger(DdfClientEx.class);
    ...
    private  final CopyOnWriteArrayList<ConnectionEventHandler> adminHandlers = new CopyOnWriteArrayList<ConnectionEventHandler>();

    // Raw DDF message handlers
    private final CopyOnWriteArrayList<FeedHandler> feedHandlers = new CopyOnWriteArrayList<FeedHandler>();

    // Market Event Handlers
    private  final CopyOnWriteArrayList<MarketEventHandler> marketEventHandlers = new CopyOnWriteArrayList<MarketEventHandler>();

    // Timestamp Handlers
    private  final CopyOnWriteArrayList<TimestampHandler> timestampHandlers = new CopyOnWriteArrayList<TimestampHandler>();

    // Quote/Market Update
    private final Map<String, CopyOnWriteArrayList<QuoteHandler>> quoteHandlers = new ConcurrentHashMap<String, CopyOnWriteArrayList<QuoteHandler>>();

    // Quote/Market Update by Exchange (ExchangeCode ==> Handler)
    private final Map<String, QuoteHandler> quoteExchangeHandlers = new ConcurrentHashMap<String, QuoteHandler>();

    // Market Depth/Book Quote
    private  final Map<String, CopyOnWriteArrayList<BookQuoteHandler>> bookQuoteHandlers = new ConcurrentHashMap<String, CopyOnWriteArrayList<BookQuoteHandler>>();
    ...
    public DdfClientEx(ClientConfig config) {
        this(config, new SymbolProviderImpl());
    }

    public DdfClientEx(ClientConfig config, SymbolProvider symbolProvider) {
        this.config = config;
    ...
    public void init() {
        ...
        DdfClientEx.UnknownSymbolThread unknownSymbolThread = new DdfClientEx.UnknownSymbolThread();
    ...
    public void connect() throws Exception {
        ...
        synchronized (dataMaster) {
            ...
            DdfClientEx.DdfClientConnectionHandler handler = new DdfClientEx.DdfClientConnectionHandler();
